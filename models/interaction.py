...
class LabManual(db.Model):
    ...
    software_used = db.Column(db.String(255), nullable=True)
    ...
    course = db.relationship('Course', backref=db.backref('lab_manuals', lazy=True, cascade='all, delete-orphan'))
    author = db.relationship('User', backref=db.backref('lab_manuals', lazy=True))

    def __repr__(self):
        return f'<LabManual {self.title}>'
